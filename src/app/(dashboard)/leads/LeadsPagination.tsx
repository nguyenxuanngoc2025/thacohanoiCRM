  ...
  return (
    <div className="flex items-center justify-between gap-2 text-slate-600">
      <div className="flex items-center gap-2 text-xs">
        ...
        <select
          className="h-6 rounded border border-slate-200 bg-white px-1 text-xs text-slate-600 focus:outline-none"
          value={pageSize}
      ...
      <div className="flex items-center gap-0.5">
        <button className={btn} disabled={page <= 1} onClick={() => onGo(page - 1)} aria-label="Trước"><ChevronLeft size={14} /></button>
        {pages(page, last).map((p, i) => p === '…'
          ? <span key={`e${i}`} className="px-0.5 text-slate-400">…</span>
          : <button
              key={p}
              className={`${btn} ${p === page ? 'text-white font-medium hover:brightness-110' : ''}`}
              style={p === page ? { backgroundColor: 'var(--color-brand)' } : undefined}
              onClick={() => onGo(p)}
            >{p}</button>)}
        <button className={btn} disabled={page >= last} onClick={() => onGo(page + 1)} aria-label="Sau"><ChevronRight size={14} /></button>
      </div>
    </div>
  );
}
